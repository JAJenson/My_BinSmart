...
$emailvalidate = True;
$unvalidate = True;
if(isset($_POST['firstname']) and isset($_POST['lastname']) and isset($_POST['username']) and isset($_POST['email']) and isset($_POST['confirme']) and isset($_POST['password']) and isset($_POST['confirmp']))
...
if ($unvalidate==True and $emailvalidate ==True){
    $servername = "localhost";
	$username = "testing";
	$password = "K-itten123";
	$dbname = "users";
	$TID = "";
	$conn = mysqli_connect($servername, $username, $password, $dbname);
	$TID = $TID .substr($fn, 0,2) .substr($ln, 0,2).substr($un, 0,3);
	$sql = "INSERT INTO `teacher_users`(`firstname`, `lastname`, `email`, `login`, `username`, `TID`) 
			VALUES ('$fn', '$ln', '$email', '$pw', '$un', '$TID')";
...
?>
<html>
<button type="button"> <a href="SignUpPage_v5.html">go back</a></button>
</html>