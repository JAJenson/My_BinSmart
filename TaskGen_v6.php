...
Function DenBinFloat() {
	...
	echo("<br> Array once te value is returned <br>");
	#print_r($DBQ);
	...
	$post= array(1,$post1,$post2,$post3,$post4,$post5);
	$bin2=0; $num2=0; $exp2=0;
	for($i=0;$i<5;$i++){
		for($j=$post[$i];$j<$post[$i+1];$j++){
			if ($i == 0){$DBQ[$j][0] ="n";}
			if ($i == 1){$DBQ[$j][0] ="r";}
			if ($i == 2){$DBQ[$j][0] ="pn";}
			if ($i == 3){$DBQ[$j][0] ="a"; $DBQ[$i][5] = mt_rand(1, ($DBQ[$i][1]-1));
				cfloat($num2,$bin2,$exp2); }
				$DBQ[$i][6]=$num2;
				$DBQ[$i][7]=$bin2;
				$DBQ[$i][8]=$exp2;
			if ($i == 4){$DBQ[$j][0] ="s"; $DBQ[$i][5] = mt_rand(1, ($DBQ[$i][1]-1)); 
				cfloat($num2,$bin2,$exp2); 
				$DBQ[$i][6]=$num2;
				$DBQ[$i][7]=$bin2;
				$DBQ[$i][8]=$exp2;}}}print_r($DBQ);
	CQ("Float", $DBQ);}
Function DenBinNorm() {
	$total =0;
	for($i=0;$i<5;$i++){
...
Function cfloat($a,$b,$c){
	...
	$exp = max($exponents_used);
	$binfloat = (1 +$binfloat)*pow(10, $exp);$binfloat =substr($binfloat, 1); 
	$a =$float;	#float num e.g. 0.x
	$b =$binfloat; # binry of float
	$c =$exp;		# exponent e.g. 8.25 ¦ 0100001 EXP[0010]
	echo("<br> ---- ".$a." -- ". $b." -- ".$c);
	return($a);
	return($b);
	return($c);
